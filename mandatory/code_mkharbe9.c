...
//     // Calculer la valeur moyenne de z
//     for (j = 0; j < fdf->map.height; j++)
//     {
//         for (i = 0; i < fdf->map.width; i++)
//         {
//             center_z += fdf->map.points[j][i].z;
//         }
//     }
//     center_z /= (fdf->map.height * fdf->map.width);
...
//         for (i = 0; i < fdf->map.width; i++)
//         {
//             // Ajuster la hauteur par rapport au centre
//             fdf->map.points[j][i].z = center_z + (fdf->map.points[j][i].z - center_z) * factor;
//         }
//     }
...
int main(int argc, char **argv)
{
	...
	if (!dim_iso(&fdf))
	{
		printf("Failed to create isometric projection\n");//changer apres
		// Nettoyage et sortie
		return 1;
	}
	//count scale
	...
	if((fdf.height)/fdf.map.iso_height <= (fdf.width)/fdf.map.iso_width)
		...
		fdf.scale = (fdf.width - 100)/fdf.map.iso_width;
	// printf("fdf.scale = %f",fdf.scale);
	// end count scale
	// printf("before iso leen[%d,%d]\n", fdf.map.iso_height, fdf.map.iso_width);
	// printf("%f",fdf.scale);
	// exit(1);
	// if(fdf.scale != 1)
	fdf.projection = 0;
	scale_points(&fdf);
	// printf("after iso leen[%d,%d]\n", fdf.height, fdf.width);
	// exit(1);
	// printf("iso leen[%d,%d]\n", fdf.map.iso_min_x, fdf.map.iso_max_x);
	// printf("iso leen[%d,%d]\n", fdf.map.iso_min_y, fdf.map.iso_max_y);
	...
	//     while(i < fdf.map.width)
	//     {
	//         printf("[%d,%d]\n",fdf.map.iso_points[j][i].x,fdf.map.iso_points[j][i].y);
	//         i++;
	//     }
	//     j++;
	// }
...
//     fdf.img = mlx_new_image(fdf.mlx, WIDTH, HEIGHT);
//     mlx_image_to_window(fdf.mlx, fdf.img, 0, 0);
//     mlx_put_pixel(fdf.img, 30, 50, 0xff4567FF);
//     mlx_loop(fdf.mlx);
// }
